...
plus ``unixname``/``uid``/``gid``. It carries no capability claims, and this
service must never derive authorization from anything beyond identity — the
broker has already authorized the call before minting the token.
...
broker blip does not take IDTOKEN issuance down with it.
"""
